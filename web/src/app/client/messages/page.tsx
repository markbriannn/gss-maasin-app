          ...
          <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg focus:ring-4 focus:ring-white/30 focus:outline-none text-gray-900 dark:text-white dark:placeholder-gray-400"
              />
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 -mt-2">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-1.5 flex mb-4">
            <button
              ...
              className={`flex-1 py-3 rounded-xl text-sm font-semibold transition-all ${!showArchived ? 'bg-gradient-to-r from-emerald-500 to-green-500 text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'
                }`}
            >
          ...
          {loadingData ? (
            <div className="space-y-3">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="bg-white dark:bg-gray-800 rounded-2xl p-4 skeleton-shimmer">
                ...
                return (
                  <div key={conversation.id} className={`relative group bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl hover:scale-[1.005] transition-all border overflow-hidden ${conversation.unreadCount > 0 ? 'border-emerald-200 dark:border-emerald-800 bg-gradient-to-r from-emerald-50/50 to-white dark:from-emerald-900/20 dark:to-gray-800' : 'border-gray-100 dark:border-gray-700'}`}>
                    ...
                    <div className="p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => router.push(`/chat/${conversation.id}`)}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`font-semibold truncate ${conversation.unreadCount > 0 ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>{conversation.otherUser?.name || 'Unknown'}</span>
                    ...
                    {menuOpen === conversation.id && (
                      <div className="absolute right-4 top-16 bg-white/90 dark:bg-gray-800/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/60 dark:border-gray-700 z-20 py-2 min-w-[170px] animate-fade-in">
                        {showArchived ? (
