...
import { MessageSquare, Search, Archive, Trash2, RotateCcw, Briefcase, Clock, CheckCheck, Users, Bell, Sparkles, MoreVertical } from 'lucide-react';
...
export default function ProviderMessagesPage() {
  ...
  const getRoleBadge = (role?: string) => {
    switch (role?.toUpperCase()) {
      case 'ADMIN': return { bg: 'bg-purple-100', text: 'text-purple-700', label: 'Support', gradient: 'from-purple-400 to-purple-600' };
  ...
  const filteredConversations = displayedConversations.filter((c) => c.otherUser.name.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <ProviderLayout>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 dark:from-gray-900 dark:via-gray-900 dark:to-gray-900">
          ...
          <div className="max-w-4xl mx-auto px-4 py-8">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-blue-100">Connect with your clients</p>
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg focus:ring-4 focus:ring-white/30 focus:outline-none text-gray-900 dark:text-white"
              />
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 -mt-2">
          {/* Tabs */}
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-1.5 flex mb-4">
            <button
              onClick={() => setShowArchived(false)}
              className={`flex-1 py-3 rounded-xl text-sm font-semibold transition-all ${!showArchived ? 'bg-gradient-to-r from-blue-500 to-indigo-500 text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'
                }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-3 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2 ${showArchived ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-md' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'
                }`}
            >
          ...
          {filteredConversations.length === 0 ? (
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-12 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-6">
                {searchQuery ? 'Try a different search term' : showArchived ? 'Archived conversations will appear here' : 'When clients message you, conversations will appear here'}
              </p>
            </div>
          ) : (
            <div className="space-y-3 pb-6">
              {filteredConversations.map((conversation) => {
                ...
                return (
                  <div key={conversation.id} className={`relative group bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl hover:scale-[1.005] transition-all border overflow-hidden ${conversation.unreadCount > 0 ? 'border-blue-200 dark:border-blue-800 bg-gradient-to-r from-blue-50/50 to-white dark:from-blue-900/20 dark:to-gray-800' : 'border-gray-100 dark:border-gray-700'}`}>
                    ...
                    <div className="p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => router.push(`/chat/${conversation.id}`)}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`font-semibold truncate ${conversation.unreadCount > 0 ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>{conversation.otherUser.name}</span>
                        ...
                        <div className="flex items-center gap-2">
                          {conversation.lastSenderId === user?.uid && <CheckCheck className={`w-4 h-4 flex-shrink-0 ${conversation.unreadCount > 0 ? 'text-gray-400' : 'text-blue-500'}`} />}
                          <p className={`text-sm truncate ${conversation.unreadCount > 0 ? 'text-gray-900 dark:text-white font-medium' : 'text-gray-500 dark:text-gray-400'}`}>{conversation.lastMessage || 'No messages yet'}</p>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-2">
                        <div className="flex items-center gap-1 text-xs text-gray-400">
                          <Clock className="w-3 h-3" />
                          <span className={conversation.unreadCount > 0 ? 'text-blue-600 font-medium' : ''}>{formatTime(conversation.lastMessageTime)}</span>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); setMenuOpen(menuOpen === conversation.id ? null : conversation.id); }} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-colors">
                          <MoreVertical className="w-5 h-5 text-gray-400" />
                        </button>
                      </div>
                    </div>
                    {menuOpen === conversation.id && (
                      <div className="absolute right-4 top-16 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/60 dark:border-gray-700 z-20 py-2 min-w-[170px] animate-fade-in">
                        {showArchived ? (
